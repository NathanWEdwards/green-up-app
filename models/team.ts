...
const defaultDate = moment(getCurrentGreenUpDay())
    .utc()
...
export default class Team {
    ...
    constructor(args: any) {
        ...
        this.created = isValidDate(new Date(args.created))
            ...
            : new Date();
        this.date = typeof args.date === 'string' ? args.date : defaultDate;
        this.description =
            typeof args.description === 'string' ? args.description : null;
        this.end = typeof args.end === 'string' ? args.end : null;
        this.id = typeof args.id === 'string' ? args.id : null;
        this.isMember =
            typeof args.isMember === 'boolean' ? args.isMember : false;
        this.isPublic =
        ...
        this.locations = Array.isArray(args.locations)
            ? args.locations.map(
                  (location: Record<string, any>): Location =>
        ...
        this.members = Object.keys(args.members || {})
            .map(
                (key: string): TeamMember =>
                    TeamMember.create({ ...args.members[key], uid: key })
            )
            .reduce(
                (
                    obj: Record<string, TeamMember>,
                    member: TeamMember
                ): Record<string, TeamMember> => ({
                    ...obj,
                    [member.uid || uuidv4()]: member
                }),
                {}
            );
        ...
        this.cleanDate =
            typeof args.cleanDate === 'string'
                ...
                : null;
        this.cleanStartTime =
            typeof args.cleanStartTime === 'string'
                ? new Date(args.cleanStartTime)
                : null;
        this.cleanEndTime =
            typeof args.cleanEndTime === 'string'

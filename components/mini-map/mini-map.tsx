...
const getLocationAsync = (): Promise<LocationCoords> => Location.requestForegroundPermissionsAsync()
...
export const MiniMap: React.FC<MiniMapProps> = ({ initialLocation, onMapClick, pinsConfig = [], style, refKey }) => {
    const [errorMessage, setErrorMessage] = useState<string | null>(null);
    const [initialMapLocation, setInitialMapLocation] = useState<any>(initialLocation);
    ...
    const placePins = (pins: MapPinConfig[] = []): React.ReactElement[] => (
        mapReady ?
            (pins || []).map(
                (pin: MapPinConfig, index: number): React.ReactElement => (
                    <Marker
    ...
    return !errorMessage
        ...
        : (
            <View style={styles.miniMap}>
                <Text style={{ minHeight: 100, minWidth: 100, height: 300, width: "100%", ...(style || {}) }}>
                    {errorMessage}
                </Text>
            </View>
        );

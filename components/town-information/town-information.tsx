...
export const TownInformation: React.FC<TownInformationProps> = ({
    hideOnError = false,
    townInfo
}) => {
        ...
        >
            ...
            {townInfo.roadsideDropOffAllowed === false && (
                <View style={styles.statusBarText}>
                    <Text>
                        {`You are in ${townInfo.name} and leaving trash bags on the roadside is`}
                    ...
                    {(townInfo.dropOffLocations || []).map(
                        (d: DropOffLocation, i: number) => (

...
interface TownLocationType {
    name?: string;
    address?: string;
    notes?: string;
}

interface TownItemData {
    key?: string;
    name?: string;
    roadsideDropOffAllowed?: boolean;
    description?: string;
    pickupLocations?: TownLocationType[];
    dropOffLocations?: TownLocationType[];
}
...
export const TownItem: React.FC<TownItemProps> = ({ item }) => (
    <View key={item.key} style={styles.infoBlock}>
        ...
        {item.description ? (
            <Text
                style={[
                    styles.textDark,
                    { fontSize: 14, marginBottom: 5, marginTop: 5 }
                ]}
            >
                {item.description}
            </Text>
        ...
        {(item.pickupLocations || []).length === 0 ? (
            <View style={styles.location}>
                <Text
                    style={[styles.textDark, { marginBottom: 5, fontSize: 14 }]}
                >
        ...
        {(item.dropOffLocations || []).length === 0 ? (
            ...
            (item.dropOffLocations || []).map(
                (loc: TownLocationType, i: number) => (
                    <View
                        key={i}
                        style={[styles.location, { borderStyle: 'solid' }]}
                    >
                        {Boolean(loc.name) ? (
                            <Text
                                style={[

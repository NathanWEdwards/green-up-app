...
const getIconName = R.cond([
    [(s: MemberStatus): boolean => s.isOwner === true, (): string => icons.OWNER],
    [(s: MemberStatus): boolean => s.status === memberStatuses.REQUEST_TO_JOIN, (): string => icons.IS_REQUESTING_TO_JOIN],
    [(s: MemberStatus): boolean => Object.keys(icons).includes(s.status), (s: MemberStatus): string => icons[s.status]],
...
export const MemberIcon: React.FC<MemberIconProps> = ({ memberStatus, style = {}, isOwner, size = 35 }) => {

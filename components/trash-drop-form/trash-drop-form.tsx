...
import { selectCurrentTown, selectCurrentTownId, selectTownData } from "@/store/slices/townsSlice";
...
export const TrashDropForm: React.FC<TrashDropFormProps> = ({ existingDrop, onSave }) => {
    ...
    const userLocation = useSelector(selectUserLocation) || {} as LocationType;
    ...
    const teamOptions: { id: string; name?: string }[] = Object.values(allTeams)
        .filter((team: any) => {
            ...
            const memberEntry = Object.values(members).find(
                (m: any) => m.uid === currentUser.uid || m.email === currentUser.email
            );
    ...
    const defaultTeam = Object.values(currentUser.teams || {})[0] || {} as any;
    ...
    const toggleTag = (tag: string): ((a: any) => any) => () => {
    ...
    const selectedSite = sitesArray.find((site: any) => site.id === drop.collectionSiteId);
    const townHasSites = sitesArray.some((site: any) => site.townId === currentTownId);
            ...
            () => (
                <View style={{ flex: 1, flexDirection: "row", justifyContent: "space-between" }}>
                    ...
                    >
                        ...
                        />
                        <View style={{ flex: 1, justifyContent: "center" }}>
                            <Text style={{ flex: 1, color: "#DDD", flexWrap: "wrap" }}>
                    ...
                    () => (
                        <Fragment>
                            ...
                            >
                                ...
                                <Text style={{ color: "white" }}>
                                    Check back to this screen when Green Up week starts to tag your
                                    bags!
                                </Text>
                            </SafeAreaView>
                        </Fragment>
                            ...
                            <ScrollView style={{ flexGrow: 1, padding: 20 }}>
                                ...
                                >
                                    ...
                                    >
                                        <TouchableOpacity
                                            onPress={() => {
                                                const bagCount = isNaN(Number(drop.bagCount))
                                                    ? 1
                                        ...
                                        <TextInput
                                            ...
                                            onChangeText={(text: string) => {
                                                const bagCount = isNaN(Number(text))
                                                    ? 1
                                                    : Number(text);
                                                setDrop({ ...drop, bagCount });
                                            }}
            ...
            >
                <SafeAreaView>
                    <SiteSelector
                        ...
                        towns={Array.isArray(townData) ? townData : Object.values(townData)}
                        close={() => {
                            setModal(null);
                        }}
                        value={selectedSite}

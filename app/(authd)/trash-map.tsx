...
const TrashMap: React.FC = () => {
    ...
    const drops: any[] = Object.values(trashDrops || {}).filter(
        (drop: any) =>
            Boolean(
                drop.location &&
    ...
    const collectionSites: any[] = R.compose(
        R.filter(
            (site: any) =>
                Boolean(site.coordinates && site.coordinates.latitude && site.coordinates.longitude)
        ),
        Object.values
    ...
    const myTrashMarkers = (drops || [])
        .filter(
            (d: any) =>
                Boolean(myTrashToggle && !d.wasCollected && d.createdBy && d.createdBy.uid === currentUser.uid)
        )
        .map((d: any) => (
            <Marker
                key={d.id}
    ...
    const uncollectedTrashMarkers = (uncollectedTrashToggle ? drops : [])
        .filter(
            (d: any) => Boolean(!d.wasCollected && d.createdBy && d.createdBy.uid !== currentUser.uid)
        )
        .map((d: any) => (
            <Marker
                key={d.id}
    ...
    const distributionSiteMarkers = (supplyPickupToggle ? distributionSites : []).map(
        (d: any, i: number) => (
            <Marker
                key={`supplyPickup${i}`}
                pinColor="green"
    ...
    const cleanAreaMarkers = (cleanAreasToggle ? cleanAreas : []).map((d: any, i: number) => (
        <Marker
            key={`cleanArea${i}`}
            pinColor="orange"
            coordinate={d.coordinates}
            stopPropagation={true}
        >
            <MultiLineMapCallout title={`${d.title}`} description={`${d.description}`} />
        </Marker>
    ));
                ...
                [
                    ...
                    () => (
                        <Fragment>
                            ...
                            >
                                <Ionicons
                                    name={Platform.OS === "ios" ? "options" : "options"}
                                    size={30}
                                    color="#888"
                                />
                            </TouchableOpacity>
                            ...
                            >
                                <View style={{ flexDirection: "row", justifyContent: "center" }}>
                                    <Text style={{ fontSize: 11, fontWeight: "bold" }}>
                                        Notice: Recording a trash drop shares your location with Greenup
                                    </Text>
                                </View>
                            </TouchableOpacity>
                            <TouchableOpacity
                                ...
                                onPress={() => router.push("/(authd)/record-trash" as any)}
                            >
                                <View style={{ flexDirection: "row", justifyContent: "center" }}>
                                    ...
                                    />
                                    <Text style={buttonStyle.label}>Record Trash Bags</Text>
                                </View>
                            </TouchableOpacity>
                        </Fragment>
                    ),

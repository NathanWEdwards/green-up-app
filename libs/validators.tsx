export const isRequired = (value: any): boolean =>
    typeof value !== 'undefined' && value !== null && value !== ''; // is Required;
export const isInRange = (min: number, max: number, value: number): boolean =>
    value <= max && value >= min;
...
export const isValidDate = (param?: any): boolean =>
    (param || 'invalid').toString() !== 'Invalid Date' && param instanceof Date;
...
export const isInTeam = (
    ...
    return (
        Object.values(teamMembers || {})
            .map((teamMember: any): string =>
                (teamMember.email || 'x').toLowerCase().trim()
            )
            .indexOf(_email) > -1
    );
};

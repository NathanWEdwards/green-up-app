...
export function searchArray<T>(keys: Array<string>, searchableItems: Array<T>, searchTerms: string): Array<T> {
    ...
    const stringsToSearchIn = (_keys: Array<string>, item: any) => _keys
        .map(key => item[key])
        .filter((term?: string): boolean => Boolean(term));

    const results = R.compose(
        R.map((scored: any): any => scored.item), // we only want items
        R.sort((scored1: any, scored2: any): number => (scored2.score - scored1.score)), // sort by score
        R.filter((scored: any): boolean => (searchTerms.trim() === "" || scored.score > 0)), // filter out zero scores
        R.map((item: any): any => ({ // get the search score for each item
            item,
            score: searchScore(
                searchTerms,

...
export const getGreenUpDayByYear = (year: number): Date => {
    ...
    let firstSaturdayInMay = moment(`${year}-05-01`);
    const firstOfMay = moment(`${year}-05-01`);
    if (firstOfMay.day() !== 6) {
        firstSaturdayInMay = moment(`${year}-05-01`)
            .startOf('week')
            .add(6, 'days');
    }
...
export function getNextGreenUpDay(today?: Date): Date {
    ...
    if (is2020) {
        const guDay2020 = '2020-05-30';
        const isBeforeGreenUpDay =
            myToday.isBefore(guDay2020) || myToday.isSame(guDay2020, 'day');
    ...
    const isBeforeGUDay =
        myToday.isBefore(guDay) || myToday.isSame(guDay, 'day');
    return isBeforeGUDay ? guDay : getGreenUpDayByYear(thisYear + 1);
}
...
export const getCurrentGreenUpDay = (today?: Date): Date => {
    ...
    return addDays(getGreenUpDayByYear(currentYear), 7) >
        (myToday as unknown as number)
...
export const greenUpWindowStart = () =>
    moment(getCurrentGreenUpDay()).subtract(1, 'days').toDate();
export const greenUpWindowEnd = () =>
    moment(getCurrentGreenUpDay()).add(4, 'days').toDate();
